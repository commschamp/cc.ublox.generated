// Generated by commsdsl2comms v7.3.0

/// @file
/// @brief Contains definition of <b>"NAV-TIMEGPS (Poll)"</b> message and its fields.

#pragma once

#include <tuple>
#include "cc_ublox/message/NavTimegpsPollCommon.h"
#include "cc_ublox/options/DefaultOptions.h"
#include "comms/MessageBase.h"

namespace cc_ublox
{

namespace message
{

/// @brief Fields of @ref NavTimegpsPoll.
/// @tparam TOpt Extra options
/// @see @ref NavTimegpsPoll
/// @headerfile cc_ublox/message/NavTimegpsPoll.h
template <typename TOpt = cc_ublox::options::DefaultOptions>
struct NavTimegpsPollFields
{
    /// @brief All the fields bundled in std::tuple.
    using All = std::tuple<
    >;
};

/// @brief Definition of <b>"NAV-TIMEGPS (Poll)"</b> message class.
/// @details
///     See @ref NavTimegpsPollFields for definition of the fields this message contains.
/// @tparam TMsgBase Base (interface) class.
/// @tparam TOpt Extra options
/// @headerfile cc_ublox/message/NavTimegpsPoll.h
template <typename TMsgBase, typename TOpt = cc_ublox::options::DefaultOptions>
class NavTimegpsPoll : public
    comms::MessageBase<
        TMsgBase,
        typename TOpt::message::NavTimegpsPoll,
        comms::option::def::StaticNumIdImpl<288 /* 0x120 */>,
        comms::option::def::FieldsImpl<typename NavTimegpsPollFields<TOpt>::All>,
        comms::option::def::MsgType<NavTimegpsPoll<TMsgBase, TOpt> >,
        comms::option::def::HasName
    >
{
    // Redefinition of the base class type
    using Base =
        comms::MessageBase<
            TMsgBase,
            typename TOpt::message::NavTimegpsPoll,
            comms::option::def::StaticNumIdImpl<288 /* 0x120 */>,
            comms::option::def::FieldsImpl<typename NavTimegpsPollFields<TOpt>::All>,
            comms::option::def::MsgType<NavTimegpsPoll<TMsgBase, TOpt> >,
            comms::option::def::HasName
        >;

public:
    // Compile time check for serialisation length.
    static const std::size_t MsgMinLen = Base::doMinLength();
    static const std::size_t MsgMaxLen = Base::doMaxLength();
    static_assert(MsgMinLen == 0, "Unexpected min serialisation length");
    static_assert(MsgMaxLen == 0, "Unexpected max serialisation length");

    /// @brief Name of the message.
    static const char* doName()
    {
        return cc_ublox::message::NavTimegpsPollCommon::name();
    }
};

} // namespace message

} // namespace cc_ublox
